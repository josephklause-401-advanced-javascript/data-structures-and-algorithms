...
describe('oops-file', () => {
  ...
  it('gets file names from a directory', ()=> {
    const files = readDir('challenges/oops-file/data');
    expect(files.length).toBe(3);
  });

  it('gets last modified date for each file', () => {
    const dateModified = getDateMod('challenges/oops-file/data/test.txt');
    expect(new Date(dateModified)).toBeInstanceOf(Date);
  });

  it('reads files', () => {
    const text = readFile('challenges/oops-file/data/test.txt');
    expect(text).toBeDefined();
  });

  it('writes files', () => {
    writeFile('challenges/oops-file/newData/test.txt');
    const text = readFile('challenges/oops-file/newData/test.txt');
    expect(text).toBeDefined();
  });

  it('returns the correct new file path using the oopsFileFunc', ()=> {
    oopsFileFunc('challenges/oops-file/data');
    const files = readDir('challenges/oops-file/data-new');
    expect(files.length).toBe(3);
